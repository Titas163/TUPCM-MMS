...
const filePath = path.join(process.cwd(), 'src/pages/admin/Subjects.tsx');
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const data = snapshot.docs.map(doc => {
        ...
        return { ...sub, className: cls?.className || 'Unknown' };
      });
      setSubjects(data);
    } catch (error) {
                ...
                <CardHeader className="border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/20 py-4 flex flex-row items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-slate-500 dark:text-slate-400">{classSubjects.length} Subjects</p>
                    </div>
                  </div>
                      ...
                      {classSubjects.map(sub => (
                        <li key={sub.subjectId} className="flex items-center justify-between p-4 hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors group">
                          <div className="flex items-center gap-3">
                            ...
                            <span className="font-medium text-slate-900 dark:text-white">{sub.subjectName}</span>
                          </div>
